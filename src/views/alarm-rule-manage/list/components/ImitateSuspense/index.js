...
// 模拟 ImitateSuspense
export class ImitateSuspense extends Component {
  state = {
    isLoading: false,
  }
  ...
  static getDerivedStateFromError(error){
    // 处理场景：选人备用UI，渲染阶段调用
    // 调用场景：渲染阶段调用，DOM未更新时，在“提交”之前调用，所以服务端也是可以调用的
    console.error('getDerivedStateFromError:', error)
  }

  componentDidCatch(error, errorInfo) {
    // 处理场景：打印错误信息
    // 调用场景：提交阶段调用，在浏览器中也可以调用
  ...
  render () {
    ...
    console.log('ImitateSuspense render isLoading:', isLoading)
